...
module.exports = function(opts) {
  ...
  // Does the remainder replace
  function replaceRemainder(text, opts) {
  ...
  function include(file, text, data, sourceFile = '') {
    ...
    function includeOnceHandler(inst) {
      ...
      if (args) {
        ...
        if (includeOnceFiles[inst.sourceFile].indexOf(args[1]) === -1) {
          ...
          return slimIncludeHandler(inst)
        } else {
          return '';
        }
      }
    ...
    function slimIncludeHandler(inst) {
      var args = /[^)"']*["']([^"']*)["'](,\s*({[\s\S]*})){0,1}\s*/.exec(inst.args)

      if (args) {
        var partialPath = path.resolve(filebase, args[1])
        var o = writePartial(inst.sourceFile, inst.before, args.input, partialPath, args[3])
        var templateArgs = JSON5.stringify(args[3] ? JSON5.parse(args[3]) : {})
        return String(" <%= render '" + o.slimRelativePath +"' " + templateArgs + " %>")
      }
    }

    function writePartial(sourceFile, beforeContent, argsInput, includePath, partialArgs) {
        ...
        /*
        if (opts.indent) {
          includeContent = setIndent(beforeContent, beforeContent.length, includeContent)
        }
        */
        ...
        if (typeof opts.filters === 'object') {
          includeContent = applyFilters(includeContent, argsInput)
        }
        ...
        recFile = include(recFile, includeContent, partialArgs ? JSON5.parse(partialArgs) : {}, sourceFile != '' ? sourceFile : currentFilename)

        if (typeof(process.env.OUTPUT_ROOT) === "undefined") {
          throw new Error("Must define OUTPUT_ROOT")
        }

        if (typeof(process.env.SOURCE_ROOT) === "undefined") {
          throw new Error("Must define SOURCE_ROOT")
        }

        var tgtRoot = process.env.OUTPUT_ROOT
        var srcRoot = process.env.SOURCE_ROOT

        var includeRelPath = path.relative(srcRoot, includePath);
        var includeOutPath = path.resolve(tgtRoot, includeRelPath)
        var includeOutDir  = path.dirname(includeOutPath)

        fs.mkdirSync(includeOutDir, {recursive: true})
        fs.writeFileSync(includeOutPath, String(recFile.contents));

        var fname = path.basename(includeRelPath);
        if (fname[0] == '_') {
          fname = fname.slice(1)
          fname = fname.slice(0, -path.extname(fname).length)
        }

      fname = path.join(path.dirname(includeRelPath), fname)

      return {
        outPath: includeOutPath,
        outPathRelative: includeRelPath,
        slimRelativePath: fname
      }
    }
